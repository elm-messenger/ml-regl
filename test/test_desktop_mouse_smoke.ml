(* Desktop-only smoke test for mouse event coordinates.
   ...
   Manual check:
   - Run the executable, resize the window to any size, then move/click inside
     it.
   - The on-screen coordinate readout and cursor marker should stay in the
     virtual 400x300 canvas range regardless of window size / high-DPI scaling.
   - A click in the visual centre should report roughly x=200, y=150.
...
let sample_label = function
  ...
  | Some s -> (
      ...
      | Some b ->
          Printf.sprintf "%s button=%d x=%.2f y=%.2f" (kind_label s.kind) b
            s.x s.y)
...
let update (m : model) (input : regl_input) :
  ...
  let m =
    ...
    | Event (MouseMove { x; y }) -> record_sample m { kind = Move; button = None; x; y }
...
let small_text x y text color =
  Regl_builtin_programs.textbox (x, y) 14.0 text "custom" color
...
let view (m : model) : Regl_common.renderable =
  ...
  let accent = if ok then Color.rgb 0.20 0.90 0.45 else Color.rgb 1.0 0.25 0.18 in
  let cursor_shapes =
    match m.last with
    | None -> []
    | Some s ->
        [
          ...
          Regl_builtin_programs.rect (s.x -. 12.0, s.y -. 1.0) (24.0, 2.0) accent;
          Regl_builtin_programs.rect (s.x -. 1.0, s.y -. 12.0) (2.0, 24.0) accent;
        ]
  in
  let failure_lines =
    m.failures |> List.rev |> List.mapi (fun i msg -> small_text 18.0 (206.0 +. (float i *. 18.0)) msg (Color.rgb 1.0 0.55 0.45))
  in
  Regl_common.group []
    ([
       ...
       Regl_builtin_programs.circle (virt_w /. 2.0, virt_h /. 2.0) 3.0 (Color.rgb 0.9 0.8 0.2);
       small_text 18.0 22.0 "mouse event virtual-coordinate smoke test" fg;
       small_text 18.0 46.0 "resize the window, then move/click: coords should stay within 0..400 / 0..300" muted;
       small_text 18.0 70.0 "left/middle/right buttons should report 1/2/3 (SDL convention)" muted;
