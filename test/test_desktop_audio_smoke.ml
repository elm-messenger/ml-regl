(* End-to-end audio assertion harness for the desktop backend.
   ...
   Loads two assets:
     - test/assets/test.ogg   (OGG Vorbis, decoded via stb_vorbis)
     - test/assets/synth.wav  (WAV, decoded via SDL3's builtin loader)

   Both are pushed through the AssetLoader → AudioEngine pipeline,
   resampled to the device rate, and registered as buffers. The
   [audio] description function builds a Regl_audio.audio tree from
   model state; the framework diffs it against the previous frame and
   ships [start_sound] / [stop_sound] / [set_volume] / etc. through
   the bridge to libdeclgl, which runs the SDL3 mixer.

   The harness keeps a private [Regl_audio.prev_state] mirror in lockstep
   with the runtime, so we can introspect the exact action stream the
   diff engine produces and assert against an expected sequence at the
   end of the run. The runtime's own diff is unaffected — both diffs
   start from [empty_state] and observe the same tree, so they emit the
   same actions modulo allocation order.

   Phases (frame numbers driven by Millisecond 10ms tick):
     -      0: load_audio for both URLs.
     -     60: start OGG.
     -    300: drop OGG volume to 0.3.
     -    400: start WAV at 2.0x playback rate.
     -    600: enable WAV loop region.
     -    800: unload OGG buffer mid-play (live release path).
     -   1000: stop everything (return Regl_audio.silence).
     -   1200: unload remaining WAV.
     -   1300: print PASS/FAIL.

   Expected action shape per phase:
     - frame 60   : StartSound (ogg)
     - frame 300  : SetVolume  (ogg, 0.30)
     - frame 350  : SetVolumeAt(ogg, 3 points)
     - frame 400  : StartSound (wav)
     - frame 600  : SetLoopConfig(wav, 200..800)
     - frame 1000 : StopSound  ×2
...
(* Each expectation is (frame, predicate, label). predicate is run on the
   action's debug string. We require exactly one match per expectation,
   in order, with frames non-decreasing. *)

let init () : model * regl_output list =
  let cmds =
    [
      config_regl (ConfigTimeInterval (Millisecond 10.0));
      start_regl
        {
          virt_width = virt_w;
          virt_height = virt_h;
...
let describe_audio (m : model) : Regl_audio.audio =
  let parts = ref [] in
  if m.ogg_started && not m.stopped then begin
    match m.ogg with
    | Ready src ->
        ...
        let a =
          if m.ogg_timeline_set then
            (* 3-point fade-in/out absolute-time envelope; values are
               relative to the OGG's anchor. *)
            let t0 = m.ogg_start_ts in
            Regl_audio.scale_volume_at
              [ (t0, 0.1); (t0 +. 500.0, 1.0); (t0 +. 2000.0, 0.0) ] a
          else a
        in
        parts := a :: !parts
    | _ -> ()
  end;
  if m.wav_started && not m.stopped then begin
    match m.wav with
    | Ready src ->
        let loop =
        ...
        in
        let cfg =
          { Regl_audio.playback_rate = 2.0; start_at = 0.0; loop }
        in
  ...
  let m', extras =
    ...
    | Event (UpdateTick ts) ->
        ...
        let m2, extras =
          ...
          end else if frame' = 300 && not m1.ogg_volume_dropped then begin
            Printf.printf "[audio-smoke] frame %d: drop OGG volume to 0.3\n%!"
              frame';
            ({ m1 with ogg_volume_dropped = true }, [])
          end else if frame' = 350 && not m1.ogg_timeline_set then begin
            Printf.printf
              "[audio-smoke] frame %d: install OGG volume timeline\n%!" frame';
            ({ m1 with ogg_timeline_set = true }, [])
          end else if frame' = 400 && not m1.wav_started then begin
            Printf.printf "[audio-smoke] frame %d: start WAV @2.0x ts=%.0fms\n%!"
              frame' m1.ts;
            ({ m1 with wav_started = true; wav_start_ts = m1.ts }, [])
          end else if frame' = 600 && not m1.wav_looped then begin
            Printf.printf
              "[audio-smoke] frame %d: enable WAV loop [200..800ms]\n%!" frame';
            ({ m1 with wav_looped = true }, [])
          end else if frame' = 800 && not m1.ogg_unloaded_live then begin
            Printf.printf
              "[audio-smoke] frame %d: unload OGG buffer mid-play\n%!" frame';
            (* Voice stays in the [describe_audio] tree, so OCaml emits
               no StopSound. The C++ ReleaseBuffer path is responsible
               for silencing the still-running voice before freeing the
               buffer. *)
            ({ m1 with ogg_unloaded_live = true }, [ unload_audio ogg_url ])
          end else if frame' = 1000 && not m1.stopped then begin
            Printf.printf "[audio-smoke] frame %d: stop everything\n%!" frame';
            ({ m1 with stopped = true }, [])
          end else if frame' = 1200 && not m1.unloaded then begin
            Printf.printf "[audio-smoke] frame %d: unload remaining WAV\n%!"
              frame';
            ({ m1 with unloaded = true }, [ unload_audio wav_url ])
          end else if frame' = 1300 && not m1.finished then begin
            Printf.printf
