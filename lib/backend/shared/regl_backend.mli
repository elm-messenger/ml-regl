(** Unified, platform-agnostic ml_regl entry point.
    ...
    This is the public interface of the virtual library [regl_backend]. Two
    implementations exist, selected at link time:
...
val create_app :
  ...
  unit
(** Spin up the application and start the platform's event/render loop. The
    exact lifecycle differs per backend:

    - JS: registers the [MlApp] global; the browser host drives
      [requestAnimationFrame]. [create_app] returns immediately.
    - Desktop: opens the SDL3 window in response to the [StartRegl] command
      emitted by [init], then enters the render loop on the OCaml main thread.
      [create_app] returns when the user closes the window. *)
